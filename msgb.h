...
struct msgb {
	...
	char *head;	/*!< \brief start of underlying memory buffer */
	char *tail;	/*!< \brief end of message in buffer */
	char *data;	/*!< \brief start of message in buffer */

	fp_QUEUE_EVENT event;
};

extern struct msgb *msgb_alloc(const char *name, char *data, uint16_t datalen, fp_QUEUE_EVENT e);
